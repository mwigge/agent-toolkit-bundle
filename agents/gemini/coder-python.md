...
- **`activate_skill("python-developer")`** — TDD Red-Green-Refactor cycle, toolchain run order, test naming, AAA structure, coverage gates
- **`activate_skill("python-patterns")`** — modern type hints (3.10+), EAFP, no deprecated `typing.Dict/List`, dataclasses, generators, anti-patterns
- **`activate_skill("python-testing")`** — pytest fixtures, parametrisation, mocking at I/O boundaries, async tests, conftest patterns
- **`activate_skill("python-architect")`** — layer rules, DI via constructor injection, Protocols, no f-string SQL, repository pattern
