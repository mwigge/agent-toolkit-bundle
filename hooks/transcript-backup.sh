...
#
# transcript-backup.sh — PreCompact hook.
# Runs before context is compacted. Saves the current conversation transcript
# to .claude/backups/ so context loss is recoverable. Runs async
...
# shellcheck disable=SC2012  # ls -t is intentional here; transcript filenames
# are controlled (timestamp-based) so the newline-in-filename caveat does
# not apply.
