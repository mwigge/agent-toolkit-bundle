...
|------|-------|------|------|-----|
| **Utility** | Devstral 24B | `title`, `summary`, `compaction` | $0 | Fast single-shot generation — no reasoning needed |
| **Primary** | Devstral 24B | All 17 local agents | $0 | Thinking mode + 128K context + native tool use |
...
## Why Devstral 24B for thinking?

A common question. Devstral 24B is a **Mixture-of-Experts** model: 8B total parameters, 4.5B active per forward pass. Despite its small active footprint it has genuine thinking capability:

- `ollama show devstral:24b` reports `Capabilities: thinking` — this is a real CoT reasoning mode, not a label
...
You would consider upgrading to a larger model (e.g. `devstral:24b` with a larger context, or `llama3.3:70b`) only if you observe consistent quality failures on complex multi-file refactors — which the `model-report` command will surface as high `tokens_reasoning` with poor output quality correlation.
...
User prompt / @agent-name
        │
        ├─ title / summary / compaction   → Devstral 24B  (utility)
        │
        ├─ All 17 local agents            → Devstral 24B          (primary)
        │   (build, plan, general,
...
### What the "Compaction · Devstral 24B" UI label means

When you see this in the OpenCode status bar:
```
▣  Compaction · Devstral 24B · 3m 17s
```
...
```
/model ollama/devstral:24b                 # switch build agent to primary tier
/model ollama/devstral:24b          # switch to utility tier
...
- `ai-dev-architecture.drawio` → "Model Routing" page — visual diagram of this tier system
- `~/.config/opencode/plugins/model-usage.ts` — instrumentation plugin source
- `~/.config/opencode/scripts/model-report.py` — aggregation script source
- `~/.config/opencode/commands/model-report.md` — slash command definition
