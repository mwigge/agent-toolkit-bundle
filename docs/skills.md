...
1. Create `agent-toolkit-bundle/skills/<skill-name>/SKILL.md`
...
**OpenCode does not ship a skill system.** There is no `~/.config/opencode/skill/` directory, no `SKILL.md` activation contract, no equivalent of the Claude Code skill loader. The `skills/` directory in this bundle is Claude-only and is never installed when the install profile is `opencode`.

If you want skill-shaped guidance to apply on OpenCode, your options are:

1. **Inline the most relevant skill content into `AGENTS.md`** so the model reads it at session start. This is verbose but reliable.
2. **Reference skills by name in agent prompts** (e.g. "use the `python` skill"). On Claude Code that resolves through the skill loader; on OpenCode the reference becomes plain prose the model reads alongside the rest of the agent body. Either way, the agent still benefits from the skill name being a stable reference.
3. **Maintain a keyword table in `AGENTS.md`** mapping prompt keywords to skill names, and instruct the model to act as if those skills were activated when a match is seen. This is the closest analogue to Claude Code's `UserPromptSubmit`-driven activation, but it relies on the model following instructions rather than a deterministic hook.
